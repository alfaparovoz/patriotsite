...
app.post('/api/articles', authenticateJWT, isWriter, upload.single('image'), async (req, res) => {
  try {
    ...
    const newArticle = new Article({
      ...
      summary,
      content,
      image: req.file ? `/uploads/${req.file.filename}` : '/images/default-article-image.jpg',
      author: req.user.id
    });
...
app.post('/login', async (req, res) => {
    try {
        ...
        res.cookie('token', token, {
            ...
            sameSite: 'lax',
            domain:'localhost',
            secure: false // Для HTTP-соединения
        });
...
app.post('/registration', upload.single('avatar'), async (req, res) => {
    try {
        ...
        sameSite: 'lax', // Или 'none', если используете HTTPS
        domain: 'localhost',
        secure: false // Для HTTP-соединений
        });
...
app.use((req, res) => {
  res.status(404).json({ error: 'Страница не найдена' }); // Возвращаем JSON, а не HTML
});
